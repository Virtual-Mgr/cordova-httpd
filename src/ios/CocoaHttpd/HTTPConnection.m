...
- (id)initWithAsyncSocket:(GCDAsyncSocket *)newSocket configuration:(HTTPConfig *)aConfig commandDelegate:(id<CDVCommandDelegate>)theCommandDelegate serverConfig:(NSDictionary *)theServerConfig
{
    if ((self = [super init]))
    {
        ...
        NSString* serveOrder = [serverConfig objectForKey:@"serve_order"];
        if (serveOrder == nil) {
            _serveOrder = FileThenWWW;
        } else {
            _serveOrder = (ServeOrder)[serveOrder intValue];
        }
    }
    return self;
}
...
-(NSString*)wwwPath:(NSString*)path
{
    if ([path hasPrefix:@"/"]) {
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    switch(_serveOrder)
    {
        default:
        case FileThenWWW:
            fullPath = [self filePath:path];
            if (fullPath == nil)
            {
                fullPath = [self wwwPath:path];
            }
            break;
